...
def ai_day_text(user, calories, burned, workouts_count, counted=True):
    ...
    if not counted:
        intro = pick([
            "Сегодня без подсчета калорий — такое бывает.",
            "День без точного учета питания, поэтому анализ будет осторожным.",
            "Калории сегодня не считались, значит смотрим в первую очередь на активность.",
            "По питанию сегодня нет точной картины, но день всё равно можно оценить."
        ])
        action = pick([
            "Завтра лучше хотя бы примерно зафиксировать питание.",
            "На завтра цель простая: вернуть учет калорий без жестких ограничений.",
            "Сделай завтра обычный понятный день: лимит калорий + любая активность.",
            "Лучший следующий шаг — спокойно вернуться к учету с первого приема пищи."
        ])
        return (
            "🧠 <b>AI-анализ дня</b>\n\n"
            f"{intro}\n"
    ...
    if calories <= goal and burned >= workout_goal:
        intro = pick([
            "🔥 Отличный день: питание под контролем и активность закрыта.",
            "🟢 Сильный день — ты удержал калории и хорошо отработал активность.",
            "💪 Сегодня прям рабочий сценарий для снижения веса.",
            "✅ Хороший баланс: лимит соблюден, движение есть, день засчитан."
        ])
        action = pick([
            "Лучшее, что можно сделать — повторить такой день завтра.",
            "Не усложняй: этот формат работает, держи такой же темп.",
            "Сегодняшний день можно брать как шаблон.",
            "Продолжай так же — без жесткача, но стабильно."
        ])
        return (
            "🧠 <b>AI-анализ дня</b>\n\n"
            f"{intro}\n"
            "Это хороший дефицит без ощущения хаоса.\n\n"
            f"🎯 {action}"
        )
    ...
    if calories <= goal and burned < workout_goal:
        intro = pick([
            ...
            "✅ Калории сегодня в порядке, это уже сильная база.",
            "🟢 С питанием справился, но активности можно было добавить.",
            "Нормальный день: питание закрыто, движение чуть ниже цели."
        ])
        action = pick([
            "Завтра добавь короткое кардио или прогулку.",
            "Следующий шаг — немного поднять активность, без перегруза.",
            "Хорошая цель на завтра: сохранить питание и добавить движение.",
            "Можно добрать активность простым способом: шаги, футбол, велик или легкая тренировка."
        ])
        return (
            "🧠 <b>AI-анализ дня</b>\n\n"
            f"{intro}\n"
            f"Сожжено {burned} ккал при ориентире около {workout_goal} ккал.\n\n"
            f"🎯 {action}"
        )

    if calories > goal and burned >= workout_goal:
        intro = pick([
            "🟡 Калории были выше плана, но тренировка частично спасла день.",
            "Небольшой перебор есть, но активность хорошо компенсировала ситуацию.",
            "День не идеальный по питанию, зато по движению всё неплохо.",
            "Ты перебрал по калориям, но не пустил день совсем мимо цели."
        ])
        action = pick([
            "Завтра не нужно жестко компенсировать — просто вернись в лимит.",
            "Лучше сделать спокойный день в рамках плана, без наказаний тренировками.",
            "Сфокусируйся завтра на еде: обычный лимит и нормальная активность.",
            "Перебор не страшен, если завтра не продолжить серию."
        ])
        return (
            "🧠 <b>AI-анализ дня</b>\n\n"
            f"{intro}\n"
            f"Съедено {calories} ккал при лимите {goal}, сожжено {burned} ккал.\n\n"
            f"🎯 {action}"
        )

    intro = pick([
        "🔴 Сегодня день ниже плана.",
        "Сегодня получился слабый день по связке питание + активность.",
        "День не провален, но по данным видно: план просел.",
        "Сегодня не лучший день для прогресса, но это нормально, если не повторять часто."
    ])
    action = pick([
        "Завтра задача простая: вернуться в лимит и добавить любую активность.",
        "Не надо компенсировать жестко. Просто сделай следующий день понятным и спокойным.",
        "Лучший ответ — не ругать себя, а закрыть завтра базу: калории + движение.",
        "Сделай завтра минимальный план: контроль еды и хотя бы легкая тренировка или прогулка."
    ])
    return (
        "🧠 <b>AI-анализ дня</b>\n\n"
        f"{intro}\n"
        f"Съедено {calories} ккал при лимите {goal}, активность: {burned} ккал.\n\n"
        f"🎯 {action}"
    )
...
async def build_week_ai_analysis(user_id):
    ...
    if good_food_days >= 5 and workout_days >= 3:
        verdict = pick([
            "🔥 Неделя сильная: питание в основном под контролем, активность хорошая.",
            "🟢 Очень достойная неделя: есть и дисциплина по еде, и движение.",
            "💪 Неделя выглядит рабочей — такой темп реально двигает к цели.",
            "✅ Хорошая неделя: большинство ключевых действий закрыты."
        ])
    elif good_food_days >= 4:
        verdict = pick([
            "🟡 Неделя нормальная: питание в целом держится, но активность можно усилить.",
            "🟡 База неплохая: по калориям есть контроль, но можно добавить движения.",
            "Нормальная неделя без провала, но есть место для усиления.",
            "Питание в целом держится, следующий фокус — стабильнее тренировки."
        ])
    elif over_days >= 3:
        verdict = pick([
            "🔴 Главная проблема недели — переборы по калориям.",
            "На этой неделе питание чаще выбивалось из плана.",
            "Вес может тормозиться из-за нескольких дней с перебором.",
            "Основная точка роста — заранее планировать дни, где обычно перебор."
        ])
    else:
        verdict = pick([
            "🟡 Неделя смешанная: есть хорошие дни, но не хватает стабильности.",
            "Неделя получилась неровной: потенциал есть, но нужна регулярность.",
            "Есть неплохие куски, но пока нет стабильной серии.",
            "Главная цель на следующую неделю — меньше разброса между днями."
        ])
...
async def main():
    ...
    scheduler.add_job(monthly_reminder_job, "cron", day=1, hour=8, minute=0)
