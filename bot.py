...
async def q(query, *args):
    async with pool.acquire() as conn:
        return await conn.execute(query, *args)

async def row(query, *args):
    async with pool.acquire() as conn:
...
async def ensure_user(user_id):
    await q("INSERT INTO users(user_id) VALUES($1) ON CONFLICT (user_id) DO NOTHING", user_id)

async def get_user(user_id):
    await ensure_user(user_id)
    return await row("SELECT * FROM users WHERE user_id=$1", user_id)
...
async def today_stats(user_id):
    today = date.today()
    report = await row("SELECT * FROM daily_reports WHERE user_id=$1 AND report_date=$2", user_id, today)
    w = await rows("SELECT * FROM workouts WHERE user_id=$1 AND workout_date=$2 ORDER BY id ASC", user_id, today)
    calories = report["calories_in"] if report else None
    counted = report["calories_counted"] if report else False
    burned = sum(x["calories_burned"] for x in w)
    return report, w, calories, burned, counted
...
async def forecast_text(user_id, user):
    ...
    ws = await rows("SELECT weight_date, weight FROM weight_logs WHERE user_id=$1 ORDER BY weight_date ASC", user_id)
    if len(ws) < 2:
        return "Пока мало данных. Внеси вес минимум 2 раза."
    ...
    kg_day = kg_lost / days
    target_date = date.today() + timedelta(days=round(left / kg_day))
    return f"📌 Осталось: {left:.1f} кг\n⚡ Темп: {kg_day*30:.1f} кг/месяц\n🎯 Прогноз цели: {target_date.strftime('%d.%m.%Y')}"
...
async def handler(message: Message):
    ...
    try:
        if state["flow"] == "registration":
        ...
        elif state["flow"] == "daily":
            await daily_flow(message, state, text)
        elif state["flow"] == "weight":
            weight = to_float(text)
            await q("UPDATE users SET current_weight=$1 WHERE user_id=$2", weight, user_id)
            await q("INSERT INTO weight_logs(user_id, weight_date, weight) VALUES($1, CURRENT_DATE, $2)", user_id, weight)
            states.pop(user_id, None)
            ...
            if state["step"] == "target_weight":
                value = to_float(text)
                await q("UPDATE users SET target_weight=$1 WHERE user_id=$2", value, user_id)
                await message.answer(f"✅ Цель обновлена: {value} кг", reply_markup=main_keyboard)
            elif state["step"] == "calories":
                value = to_int(text)
                await q("UPDATE users SET daily_calorie_goal=$1 WHERE user_id=$2", value, user_id)
...
async def registration_flow(message, state, text):
    ...
    if step == "start_weight":
        ...
        height_cm=$3, age=$4, daily_calorie_goal=$5, is_registered=TRUE
        WHERE user_id=$6
        """, data["start_weight"], data["target_weight"], data["height"], data["age"], data["calories"], user_id)
        await q("INSERT INTO weight_logs(user_id, weight_date, weight) VALUES($1, CURRENT_DATE, $2)", user_id, data["start_weight"])
        states.pop(user_id, None)
...
async def daily_flow(message, state, text):
    ...
    if step == "calories":
        ...
        if text.startswith("0"):
            ...
            await message.answer("Выбери 0, 1 или 2 тренировки.")
...
async def save_daily(message, data):
    ...
    await q("DELETE FROM workouts WHERE user_id=$1 AND workout_date=CURRENT_DATE", user_id)
    for w in data.get("workouts", []):
        await q("INSERT INTO workouts(user_id, workout_date, workout_type, calories_burned) VALUES($1, CURRENT_DATE, $2, $3)", user_id, w["type"], w["calories"])
    states.pop(user_id, None)
...
async def main():
    ...
    await create_tables()
    print("Fitness bot PostgreSQL full version started")
